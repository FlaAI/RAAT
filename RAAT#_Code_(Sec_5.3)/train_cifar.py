...
def main():
    ...
    train_batches = Batches(train_set, args.batch_size, shuffle=True, num_workers=2)
    ...
    def apply_transforms(batch_imgs_gpu):
        B, C, H, W = batch_imgs_gpu.shape

        batch_imgs_cpu = batch_imgs_gpu.cpu()

        out_imgs_tensors = []
        for j in range(B):
            img_cpu_tensor = batch_imgs_cpu[j]
            img_numpy = img_cpu_tensor.numpy()

            out_h, out_w = t_crop.h, t_crop.w
            margin_h = H - out_h
            margin_w = W - out_w

            x0 = torch.randint(0, margin_w + 1, size=(1,)).item()
            y0 = torch.randint(0, margin_h + 1, size=(1,)).item()
            do_flip = torch.rand(1).item() > 0.5

            img_cropped_numpy = t_crop(img_numpy, x0, y0)
            img_flipped_numpy = t_flip(img_cropped_numpy, do_flip)

            img_final_tensor = torch.from_numpy(img_flipped_numpy)

            out_imgs_tensors.append(img_final_tensor)

        return torch.stack(out_imgs_tensors).to(batch_imgs_gpu.device)
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for i, batch in enumerate(train_batches):
            if args.eval:
                break
            X_orig, y = batch['input'], batch['target']

            X = apply_transforms(X_orig)
            X_aug = apply_transforms(X_orig)
            ...
            if args.attack == 'pgd':
                if not args.stronger_attack or epoch < args.stage1:  # ReBAT[strong]
                    ...
                    delta_aug = attack_pgd(model, X_aug, y, epsilon, pgd_alpha, args.attack_iters, args.restarts, args.norm)
                else:  # ReBAT (without stronger attack)
                    delta = attack_pgd(model, X, y, args.stronger_epsilon / 255, pgd_alpha,
                                       args.stronger_attack_iters, args.restarts, args.norm)
                    delta_aug = attack_pgd(model, X_aug, y, args.stronger_epsilon / 255, pgd_alpha,
                                           args.stronger_attack_iters, args.restarts, args.norm)
                delta = delta.detach()
                delta_aug = delta_aug.detach()
            elif args.attack == 'none':  # standard training
                delta = torch.zeros_like(X)
                delta_aug = torch.zeros_like(X_aug)

            ### ----- DICAR ----- ###

            with (torch.no_grad()):
                eval_output_benign = model_wa(normalize(torch.clamp(X, min=lower_limit, max=upper_limit)))
                eval_predicted_labels_benign = torch.argmax(eval_output_benign, 1).cpu().data.numpy()

                eval_output_adv = model_wa(normalize(torch.clamp(X + delta[:X.size(0)], min=lower_limit, max=upper_limit)))
                eval_predicted_labels_adv = torch.argmax(eval_output_adv, 1).cpu().data.numpy()

                adv_input_reduced = normalize(torch.clamp(X + 0.1 * delta[:X.size(0)], min=lower_limit, max=upper_limit)).clone().detach()
                eval_output_adv_reduced = model_wa(adv_input_reduced)
            ...
            data_ce = torch.cat((
                normalize(torch.clamp(X + delta[:X.size(0)], min=lower_limit, max=upper_limit))[non_boundary_index],
                normalize(torch.clamp(X + 0.1 * delta[:X.size(0)], min=lower_limit, max=upper_limit))[boundary_index],
                normalize(torch.clamp(X + delta[:X.size(0)], min=lower_limit, max=upper_limit))[misclassified_index]))
            labels_ce = torch.cat((
            ...
            with (torch.no_grad()):
                eval_output_benign_aug = model_wa(normalize(torch.clamp(X_aug, min=lower_limit, max=upper_limit)))
                eval_predicted_labels_benign_aug = torch.argmax(eval_output_benign_aug, 1).cpu().data.numpy()

            misclassified_index_aug = all_index[eval_predicted_labels_benign_aug != labels_np]
            ori_misclassified_index_set = set(misclassified_index) | set(misclassified_index_aug)

            ori_train_len = int(len(labels_np))
            ...
            if len(ICT_index) > 0:
                _lambda = np.random.beta(0.75, 0.75)
                mixup_rate = max(_lambda, (1 - _lambda))
                ICT_data = (mixup_rate * normalize(torch.clamp(X + delta[:X.size(0)], min=lower_limit, max=upper_limit))[ICT_index]
                            + (1 - mixup_rate) * normalize(torch.clamp(X_aug + delta_aug[:X.size(0)], min=lower_limit, max=upper_limit))[ICT_index])
                ICT_output_1 = model(ICT_data)

                output_benign = model(normalize(torch.clamp(X, min=lower_limit, max=upper_limit)))
                output_benign_aug = model(normalize(torch.clamp(X_aug, min=lower_limit, max=upper_limit)))
                ICT_output_2 = (mixup_rate * output_benign[ICT_index] + (1 - mixup_rate) * output_benign_aug[ICT_index])

                loss_con = 1.0 * _jensen_shannon_div(ICT_output_1, ICT_output_2, 0.5)

                if loss_con < 1e10:
                    if args.use_reg_schedule:
                        beta = reg_schedule(epoch + (i + 1) / len(train_batches))
                    robust_loss += loss_con * beta
